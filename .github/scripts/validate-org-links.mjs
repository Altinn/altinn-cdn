    ...
    for (const field of fields) {
      ...
      if (url && url.length > 0) {
        if (!url.startsWith(CDN_BASE_URL)) {
          console.error(
            `Error: Org '${orgKey}' has ${field} URL that does not start with ${CDN_BASE_URL}: ${url}`,
          );
          errors++;
          continue;
        }
        ...
        if (!filePath.startsWith(absoluteRepoRoot + path.sep)) {
          console.error(
            `Error: Org '${orgKey}' has ${field} URL with invalid path (traversal detected): ${url}`,
          );
          errors++;
          continue;
        }
        ...
        if (!fs.existsSync(filePath)) {
          console.error(
            `Error: Org '${orgKey}' has ${field} link to missing file: ${url} (Expected file at: ${relativePath})`,
          );
          errors++;
        }
      }
...
Validation failed with ${errors} error(s).`);
  ...
  console.log(
    "Validation successful! All logo and emblem links are valid and point to existing files.",
  );
}
